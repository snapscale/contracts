...
void ca::addcrl(public_key pk)
{
    require_auth( get_self() );

    check(is_genesis_key(pk)==false, "Not allowed add the genesis key to CRL");

    crls statstable( get_self(), get_self().value ); // code, scope

    auto hpk = getKeyHash(pk);
    auto crl_table = statstable.get_index<"bypk"_n>();
    auto crl = crl_table.find(hpk);
...
// EOSIO_DISPATCH(eosio::ca, (addmem)(changestatus)(addcert)(addcrl)(delmem)(delcrl)(delallmem))